...
internal static class PluginLoc
{
    ...
    public static readonly Dictionary<string, string> Builtin = new()
    {
        ...
        ["gen_platform"]   = "Target platform",
        ["gen_platform_tip"] = "Only for C/C++: hints the AI which platform's APIs to use. 'Portable' avoids OS-specific calls (or guards them per platform).",
        ["plat_portable"]  = "Portable / Standard",
        ["gen_multifile"]  = "One file per type (multi-file project)",
        ["gen_multifile_tip"] = "Emit one source file per class/type (plus a header/functions file) instead of a single file — cleaner projects, smaller AI requests, and required for Java. Not available for Verse.",
        ["gen_basewarn"]   = "⚠ This looks like a BASE model (not instruction-tuned). Base models ignore the prompt and produce garbage for code generation — pick an instruct/chat model instead.",
        ...
        ["gen_target"]     = "Target folder (defaults to the project's Code folder)",
        ["gen_browse"]     = "Browse…",
        ["gen_notarget"]   = "Please set a target folder.",
        ["gen_skeleton"]   = "Generating skeleton…",
        ["gen_filling"]    = "AI filling {0} …",
        ["gen_includes"]   = "Checking includes for {0} …",
        ...
        ["gen_failed"]     = "Code generation failed.",
        ["gen_written"]    = "Written to:\n{0}\n\n{1} file(s).{2}{3}",
        ["gen_build"]      = "Build:",
        ["gen_replaced"]   = "Replaced {0} existing file(s). Their previous versions were moved to the '{1}' folder — note: only the most recently replaced files are kept there, and they are overwritten on the next generation:\n{2}",
